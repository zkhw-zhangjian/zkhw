...
namespace zkhwClient.dao
{
    class psychiatricPatientServicesDaoS
    {
        public DataTable queryPsychosis_follow_record(string pCa, string time1, string time2, string code)
        {
            ...
            sql += ") bb LEFT JOIN(select id,archive_no,max(visit_date) visit_date,visit_doctor,next_visit_date from psychosis_follow_record where visit_date >= '" + time1 + "' and visit_date <= '" + time2 + "') aa on bb.archive_no = aa.archive_no";
