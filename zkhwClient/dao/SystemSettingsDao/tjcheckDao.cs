...
namespace zkhwClient.dao
{
    class tjcheckDao
    {
        ...
        //修改血常规检查信息根据档案号和条码号
        public bool updateXuechangguiInfo(string aichive_no, string barcode, string WBC, string RBC, string PCT, string PLT, string HGB, string HCT, string MCV, string MCH, string MCHC, string RDWCV, string RDWSD, string MONO, string MONOP, string GRAN, string GRANP, string NEUT, string NEUTP, string EO, string EOP, string BASO, string BASOP, string LYM, string LYMP, string MPV, string PDW, string MXD, string MXDP, string PLCR, string OTHERS,string deviceno,string xcg)
        {
            int ret = 0; 
            String sql = "update zkhw_tj_xcg set WBC='" + WBC + "',RBC='" + RBC + "',PCT='" + PCT + "',PLT='" + PLT + "',HGB='" + HGB + "',HCT='" + HCT + "',MCV='" + MCV + "',MCH='" + MCH + "',MCHC='" + MCHC + "',RDWCV='" + RDWCV + "',RDWSD='" + RDWSD + "',MONO='" + MONO + "',MONOP='" + MONOP + "',GRAN='" + GRAN + "',GRANP='" + GRANP + "',NEUT='" + NEUT + "',NEUTP='" + NEUTP + "',EO='" + EO + "',EOP='" + EOP + "',BASO='" + BASO + "',BASOP='" + BASOP + "',LYM='" + LYM + "',LYMP='" + LYMP + "',MPV='" + MPV + "',PDW='" + PDW + "',MXD='" + MXD + "',MXDP='" + MXDP + "',PLCR='" + PLCR + "',OTHERS='" + OTHERS + "',deviceModel='"+deviceno+ "' ZrysXCG='"+ xcg + "' where aichive_no = '" + aichive_no + "' and bar_code='" + barcode + "'";
            ret = DbHelperMySQL.ExecuteSql(sql);
            return ret == 0 ? false : true;
        }
        //查询尿常规检查信息根据档案号和条码号
